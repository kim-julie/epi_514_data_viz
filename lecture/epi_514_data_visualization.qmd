...
-   Based on the principles by Edward Tufte, discuss potential problems of this chart with your table members
...
::: incremental
-   Be NICE to your viewers - never assume that people will know what you're trying to show
...
-   Think about the morale of the story before you plot.
    ...
    -   Many journals only allow 4-6 tables/figures combined in the main text - be strategic about what you want to show.
...
::: incremental
-   At some point of your review process, the reviewers or editors will comment about image quality, captions, choice of colors and axis labels/scale/titles.

    -   If image quality is too low, it might appear blurry in your paper.

    -   Provide details for caption

    -   Add details even if you included in your main text.
...
-   `geom`: short for geometry - specify type of plot
    ...
    -   Different geometry components require different argument
        -   e.g. `geom_ribbon` allows you to plot area - this requires `xmin` and `xmax` and/or `ymin` and `ymax`
:::
...
brfss <- read_csv("/Users/sojink/Documents/GitHub/epi_514_data_viz/data/individualhw_post.csv") # Change directory and filename to your choice
```
...
-   Based on what we know about `avedrnk3` variable, we would rather want to look at closer look at numbers between 0-20, instead of 0-8 0
-   We will add more arguments in:
    -   `scale_x_continuous`: for x-axis labels
    -   argument: `limits` to specify values for axis limits
...
-   For this course scatter plots do not really make sense - why?

-   But we'll still learn how to do this
...
```{r, message = F, warning = T}
ggplot(data = brfss, mapping = aes(x = avedrnk3, y = marijan1))+
  geom_point()+
...
```{r, message = F, warning = T}
ggplot(data = brfss, mapping = aes(x = avedrnk3, y = marijan1, col = age_g))+
  geom_point()+
  labs(x = "Average alcoholic drinks per day ", y = "Days of Marijuana use")+
  facet_wrap(~sexvar)
```
...
ggplot(data = brfss, mapping = aes(x = avedrnk3, y = marijan1, shape = age_g))+
  geom_point()+
  labs(x = "Average alcoholic drinks per day", y = "Marijuana use")+
  facet_wrap(~sexvar)
```
...
```{r, message = F, warning = T}
ggplot(data = brfss, mapping = aes(x = avedrnk3, y = marijan1, shape = age_g))+ 
  geom_point()+ labs(x = "Average alcoholic drinks per day", y = "Marijuana use")+ 
  facet_wrap(~sexvar)+
...
brfss%>%
  ...
  mutate(denom = sum(num), # calculate count for all race by sex
         proportion = round(100 * num/denom,2))%>%
  knitr::kable()
```
...
brfss%>%
  ...
  ggplot()+
  geom_bar(aes(x = sexvar, y = 100*proportion, fill = imprace), stat = 'identity')
```
...
-   Talk to your neighbors - what questions do you have?

![](images/Neta1.jpg){width="375"}
